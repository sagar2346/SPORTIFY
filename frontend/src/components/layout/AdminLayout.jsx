...
import { FiUsers, FiGrid, FiMail, FiCheck, FiVideo, FiAward, FiShield, FiHome, FiSearch, FiBell, FiChevronRight, FiMenu, FiX } from 'react-icons/fi';
...
const AdminLayout = () => {
    ...
    const navLinks = [
        ...
        { path: '/admin/kyc-requests', label: 'KYC Requests', icon: FiCheck },
        { path: '/admin/teams', label: 'Teams', icon: FiShield },
        { path: '/admin/teams-chat', label: 'Team Groups', icon: FiUsers },
        { path: '/admin/tournaments', label: 'Tournaments', icon: FiAward },
        { path: '/admin/messages', label: 'Customer Support', icon: FiMail },
    ];
            ...
            `}>
                ...
                <nav className="flex-1 overflow-y-auto px-4 py-8 space-y-2 custom-scrollbar">
                    <div className="px-5 mb-4">
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">Administration</p>
                    </div>
                    {navLinks.map((link) => (
                        <Link
                            key={link.path}
            ...
            <div className="flex-1 flex flex-col min-w-0 bg-gray-100 overflow-hidden">
                ...
                <header className="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-8 shrink-0">
                    <div className="flex items-center gap-4 text-sm font-bold text-gray-400 uppercase tracking-widest">
                        <span className="text-gray-900">Admin Control</span>
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="text-left hidden sm:block">
                                <p className="text-xs font-black text-gray-900 uppercase tracking-wider">{user?.name || 'Administrator'}</p>
                                <p className="text-[10px] font-bold text-rose-500 uppercase tracking-widest">{user?.role === 'admin' ? 'Master Admin' : 'Staff'}</p>
                            </div>
                        </div>
                    </div>
                </header>
                ...
                <main className={`
                    ...
                    ${isActive('/admin/teams-chat') ? 'p-0' : 'p-8 lg:p-12'}
                `}>
                    <div className={`${isActive('/admin/teams-chat') ? 'max-w-none h-full' : 'max-w-7xl mx-auto'}`}>
                        <Outlet />
                    </div>
                </main>
            </div>
