    ...
    lines.forEach((line, idx) => {
        ...
        if (trimmed.includes('EMAIL_USER') || trimmed.includes('EMAIL_PASS')) {
            ...
            if ((val.startsWith('"') && val.endsWith('"')) || (val.startsWith("'") && val.endsWith("'"))) {
                console.log('⚠️  WARNING: Value is wrapped in quotes. Nodemailer usually handles this, but try removing them if it fails.');
                val = val.slice(1, -1);
            }
            ...
            if (key === 'EMAIL_PASS') {
                passFound = true;
                if (val.length < 16) {
                    console.log(`⚠️  WARNING: Password is only ${val.length} chars. App Passwords are usually 16 chars.`);
                }
                if (val.includes(' ')) {
                    console.log('ℹ️  INFO: Password contains spaces. This is usually fine, but if it fails, try removing them.');
                }
            }
        }
    });
